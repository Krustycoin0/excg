        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          {account ? (
            ...
            <button 
              ...
              style={{ background: '#10b981', color: 'white', border: 'none', padding: '10px 20px', borderRadius: '6px', cursor: 'pointer' }}
            >
              Connetti Wallet
            </button>
          )}
...
// Componente Swap semplificato al massimo
const SimpleSwap = ({ account }) => {
  // Crea un elemento div per il widget
  useEffect(() => {
    if (account && window.LiFiWidget) {
      // Il widget si caricherà automaticamente
      console.log('Account ready for widget:', account);
    }
  }, [account]);
